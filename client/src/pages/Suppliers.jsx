    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-md shadow-slate-100/50 animate-fade-in-up flex flex-col items-start" style={{ animationDelay: '0.1s' }}>
          <div className="p-2 bg-indigo-50 rounded-xl mb-3">
          ...
          {!isMobile && (
            <div className="flex items-center gap-2">
              <button
                ...
                className={`p-2 rounded-lg transition ${viewMode === 'grid' ? 'bg-orange-100 text-orange-600' : 'text-slate-400 hover:bg-slate-100'}`}
              ...
              <button
                ...
                className={`p-2 rounded-lg transition ${viewMode === 'list' ? 'bg-orange-100 text-orange-600' : 'text-slate-400 hover:bg-slate-100'}`}
                title="Vista lista"
              >
      ...
      {effectiveViewMode === 'grid' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredSuppliers.map((supplier, index) => (
            <div key={supplier.id} className="bg-white rounded-2xl shadow-md shadow-slate-100/50 p-5 group animate-fade-in-up" style={{ animationDelay: `${index * 0.05}s` }}>
              ...
              {!supplier.contact && !supplier.phone && !supplier.email && !supplier.address && (
                <p className="text-sm text-slate-400 italic mt-2">Sin información de contacto</p>
              )}
            </div>
          ))}
        </div>
        ...
        <div className={`fixed inset-0 z-50 flex ${isMobile ? 'items-end' : 'items-center justify-center'} bg-black/30 backdrop-blur-sm`} onClick={() => setShowModal(false)}>
          <div className={`bg-white w-full max-w-md flex flex-col shadow-2xl ${
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-4 space-y-4 modal-scroll" style={{ paddingBottom: isMobile ? 'calc(80px + env(safe-area-inset-bottom))' : '16px' }}>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">✉️ Email</label>
                <input
                  type="email"
