...
export const AuthProvider = ({ children }) => {
  ...
  const addMovement = useCallback(async (data) => {
    ...
    if (user?.role !== 'ADMIN') sendPushNotification({ title: '🔄 Movimiento Realizado', body: `${user?.name} registró ${data.type} de ${data.quantity} ${product.unit || ''} de ${product.name}`, url: '/movements' });
    if (newStock <= product.min_stock && product.min_stock > 0) sendPushNotification({ title: '⚠️ Stock Bajo', body: `${product.name} tiene solo ${newStock} ${product.unit} (mín: ${product.min_stock})`, url: '/inventory' });
